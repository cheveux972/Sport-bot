...
OUTPUT_DIR = Path(**file**).parent.parent / “data”
POLL_INTERVAL_S = 60
CACHE_MAX_AGE_S = 300
...
async def _tick(self):
    try:
        ...
        if matches:
            ...
            self._save()
            log.info(f"Captures: {len(matches)} matchs")
        else:
            log.warning("Capture vide - conservation du cache")

    except Exception as exc:
        log.error(f"Erreur capture: {exc}")
